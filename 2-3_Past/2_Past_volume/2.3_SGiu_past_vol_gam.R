...
moving_res %>% 
  group_by(Date) %>% 
  mutate(predict_gam_random1_avrg=mean(gam_v1),
         predict_gam_random2_avrg=mean(gam_v2),
         predict_gam_random3_avrg=mean(gam_v3),
         predict_gam_random4_avrg=mean(gam_v4),
         predict_gam_random5_avrg=mean(gam_v5)) %>%
  distinct(Date, , .keep_all = T) %>% 
